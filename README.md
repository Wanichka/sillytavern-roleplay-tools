...
## Переход с тестовой версии

1. В менеджере расширений Tavern у **всех семи расширений** из таблицы нажми **Switch branch / Сменить ветку** и выбери `main` или `origin/main`.
2. После переключения **обнови каждое расширение** обычной кнопкой обновления: локальная ветка `main` может содержать старую версию.
3. Полностью обнови страницу Tavern. Сверь версии с таблицей ниже.

Переустановка, удаление папок или очистка данных сайта не нужны. Раскладка, размеры и данные продолжают использовать прежние хранилища. Тестовые ветки `roleplay-tools-integration` оставлены для истории; установленные копии должны быть переключены на `main`, чтобы получать обычные обновления.
